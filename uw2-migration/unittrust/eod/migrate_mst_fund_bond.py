"""Migrate MstFundBond from UnitTrust MSSQL to UWealth v2 PostgreSQL.

All legacy columns map directly to ``fund_upstream.mst_fund_bond``. Writes use
the legacy ``fund_id`` primary key, so the migration is safe to run repeatedly.

Examples:
    python migrate_mst_fund_bond.py --count-only
    python migrate_mst_fund_bond.py --batch-size 500
"""
...
SOURCE_TABLE = "dbo.MstFundBond"
TARGET_SCHEMA = "fund_upstream"
TARGET_TABLE = "mst_fund_bond"
...
KEY_COLUMNS: Tuple[str, ...] = ("fund_id",)


def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Migrate UnitTrust MstFundBond to UWealth v2 PostgreSQL"
    )
...
def upsert_sql(row_count: int) -> str:
    ...
    return (
        ...
        f"VALUES {', '.join([row_placeholders] * row_count)} "
...
def verify_target_table(connection) -> None:
    ...
    try:
        cursor.execute("SELECT to_regclass(%s)", (TARGET_QUALIFIED_TABLE,))
        if not cursor.fetchone()[0]:
            raise RuntimeError(
                f"Target table {TARGET_QUALIFIED_TABLE} does not exist. "
                "Apply the v2 database migrations first."
            )
